...
function AdminDataDosenPage() {
    ...
    const fetchData = async (page = 1, query = '') => {
        setLoading(true);
        try {
            const response = await api.get('/api/user/dosen', {
                params: {
                    page: page - 1,
                    limit: itemsPerPage,
                    search_query: query,
    ...
    const handleDelete = async () => {
        if (selectedDosen) {
            try {
                await api.delete(`/api/user/${selectedDosen.userId}/delete`, {
                    headers: {
                        Authorization: `Bearer ${secureLocalStorage.getItem('accessToken')}`,
                    },
                });
    ...
    return (
        <div className='h-screen flex flex-col'>
            ...
            <DeleteConfirmationModal
                ...
                header={'Hapus Dosen'}
                message={`Apakah Anda yakin ingin menghapus Dosen: "${selectedDosen?.name}?"`}
            />
        </div>
    );
}

export default AdminDataDosenPage;
